...
const Logo = ({ logoImage, logoText = "PhimBro" }) => {
      ...
      {logoImage && !imageError ? (
        ...
        <div className="relative">
          <img 
            src={logoImage} 
            alt="Logo" 
            className="w-16 h-16 rounded-lg shadow-lg group-hover:shadow-cyan-400/50 transition-all duration-300 object-cover border-2 border-transparent group-hover:border-cyan-300/30"
            onError={handleImageError}
          />
          {/* Hiệu ứng gradient overlay khi hover */}
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-300/20 via-emerald-300/20 to-pink-300/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
          {/* Glow effect */}
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-300/30 via-emerald-300/30 to-pink-300/30 blur-lg rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10"></div>
        </div>
      ) : (
        // Logo text fallback với gradient
        <div className="text-3xl font-bold cursor-pointer">
          <span className="bg-gradient-to-r from-cyan-300 via-emerald-300 to-pink-300 bg-clip-text text-transparent group-hover:from-cyan-400 group-hover:via-emerald-400 group-hover:to-pink-400 transition-all duration-300 font-serif italic">
            {logoText}
          </span>
          {/* Hiệu ứng glow cho text */}
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-300/20 via-emerald-300/20 to-pink-300/20 blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10"></div>
        </div>
      )}
    </div>
  );
...
const Header = () => {
  ...
  const handleLogoUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        // Lưu vào localStorage để giữ logo sau khi reload
  ...
  // Function để reset về logo mặc định
  const resetToDefaultLogo = () => {
    setLogoImage("https://www.rophim.mx/images/logo.svg");
    localStorage.removeItem('customLogo');
  };

  // Load logo từ localStorage khi component mount, nếu không có thì dùng logo mặc định
  useEffect(() => {
    const savedLogo = localStorage.getItem('customLogo');
    if (savedLogo) {
      setLogoImage(savedLogo);
    }
    // Nếu không có logo custom, sử dụng logo mặc định từ rophim.mx
  }, []);
          ...
          <div className="flex items-center space-x-8">
            ...
            <div className="flex items-center space-x-2">
              <Link to="/" className="flex items-center group">
                <Logo logoImage={logoImage} logoText={logoText} />
              </Link>
              ...
              <div className="flex items-center space-x-1">
                ...
                <div className="relative">
                  ...
                  <label
                    htmlFor="logo-upload"
                    className="flex items-center justify-center w-8 h-8 bg-gray-700 hover:bg-gray-600 rounded-full cursor-pointer transition-colors duration-200 group/upload"
                    title="Upload logo"
                  >
                    <svg className="w-4 h-4 text-gray-300 group-hover/upload:text-white transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  className="flex items-center justify-center w-8 h-8 bg-gray-700 hover:bg-gray-600 rounded-full cursor-pointer transition-colors duration-200 group/reset"
                  title="Reset to default logo"
                >
                  <svg className="w-4 h-4 text-gray-300 group-hover/reset:text-white transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
