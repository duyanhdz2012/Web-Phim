...
export const movieApi = {
      ...
      while (quickFiltered.length < 64 && currentPage <= maxQuickPages) {
        try {
          ...
          if (pageData.items && Array.isArray(pageData.items)) {
            // Lọc chỉ những phim KHỚP với từ khóa trong TÊN PHIM
            const pageFiltered = pageData.items.filter(movie => {
              // QUAN TRỌNG: Chỉ tìm trong TÊN PHIM, không tìm trong thể loại
              const movieName = (movie.name || '').toLowerCase();
              const keywordLower = keyword.toLowerCase();
              
              // CHỈ LẤY PHIM CÓ TÊN CHỨA TỪ KHÓA
              if (!movieName.includes(keywordLower)) {
                return false;
              }
              
              console.log(`Found matching movie by name: "${movie.name}" contains "${keyword}"`);
              
              // Additional filters (chỉ áp dụng nếu có)
              if (category) {
  ...
  getMoviesByCategoryNew: async (category, options = {}) => {
    try {
      ...
      // Bước 1: Tìm kiếm từng trang cho đến khi có đủ 64 phim KHỚP với thể loại
      console.log('Step 1: Searching page by page until we find 64 matching movies...');
      let quickFiltered = [];
      let currentPage = 1;
      const maxQuickPages = 50; // Tăng lên 50 trang để tìm đủ 64 phim khớp
      ...
      while (quickFiltered.length < 64 && currentPage <= maxQuickPages) {
        try {
          console.log(`Searching page ${currentPage} for ${category} movies...`);
          const pageData = await movieApi.getNewMovies(currentPage);
          
          if (pageData.items && Array.isArray(pageData.items)) {
            // Lọc chỉ những phim KHỚP với thể loại
            const pageFiltered = pageData.items.filter(movie => {
              const movieCategories = movie.category || [];
              const categoryNames = movieCategories.map(cat => cat.name || cat).join(' ').toLowerCase();
            ...
            // CHỈ THÊM NHỮNG PHIM KHỚP VÀO KẾT QUẢ
            quickFiltered = quickFiltered.concat(pageFiltered);
            console.log(`Page ${currentPage}: found ${pageFiltered.length} matching ${category} movies, total so far: ${quickFiltered.length}`);
            
            // Nếu đã có đủ 64 phim KHỚP, dừng lại
            if (quickFiltered.length >= 64) {
              console.log(`Found enough matching ${category} movies (${quickFiltered.length}), stopping search`);
              break;
            }
          }
