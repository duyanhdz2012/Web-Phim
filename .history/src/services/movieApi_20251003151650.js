      ...
      while (quickFiltered.length < 64 && currentPage <= maxQuickPages) {
        try {
          ...
          if (pageData.items && Array.isArray(pageData.items)) {
            ...
            console.log(`Page ${currentPage}: found ${pageFiltered.length} "${keyword}" movies, total so far: ${quickFiltered.length}`);
            
            // Nếu đã có đủ 64 phim, dừng lại
            if (quickFiltered.length >= 64) {
              console.log(`Found enough "${keyword}" movies (${quickFiltered.length}), stopping search`);
              break;
            }
          }
