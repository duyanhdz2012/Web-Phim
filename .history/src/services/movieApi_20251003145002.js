...
export const movieApi = {
  ...
  searchMovies: async (keyword, options = {}) => {
    try {
      ...
      console.log(`Searching movies with keyword "${keyword}" - using new movies API with client-side filtering`);
      
      // Search through multiple pages to find more results
      let allMovies = [];
      let currentPage = 1;
      ...
      console.log(`Searched through ${currentPage - 1} pages, found ${allMovies.length} total movies`);
  ...
  getMoviesByCategoryNew: async (category, options = {}) => {
    try {
      ...
      // Use working API and filter client-side
      console.log(`Fetching movies by category ${category} - using new movies API with client-side filtering`);
  ...
  getMoviesByCountry: async (country, options = {}) => {
    try {
      ...
      // Use working API and filter client-side
      console.log(`Fetching movies by country ${country} - using new movies API with client-side filtering`);
      const data = await movieApi.getNewMovies(page);
  ...
  getMoviesByYearNew: async (year, options = {}) => {
    try {
      ...
      // Use working API and filter client-side
      console.log(`Fetching movies by year ${year} - using new movies API with client-side filtering`);
      const data = await movieApi.getNewMovies(page);
