...
export const movieApi = {
  ...
  // Lấy tất cả phim từ database với caching
  getAllMovies: async () => {
    try {
      // Kiểm tra cache
      const now = Date.now();
      ...
      console.log('Fetching movies from database (first 10 pages for performance)...');
      let allMovies = [];
      let currentPage = 1;
      const maxPages = 10; // Giới hạn 10 trang đầu để tối ưu hiệu suất
      ...
      while (currentPage <= maxPages) {
        try {
          ...
          if (data.items && Array.isArray(data.items) && data.items.length > 0) {
            ...
            console.log(`Fetched page ${currentPage}, got ${data.items.length} items, total so far: ${allMovies.length}`);
            currentPage++;
          } else {
      ...
      console.log(`Fetched movies: ${allMovies.length} total from ${currentPage - 1} pages (cached for 5 minutes)`);
      return allMovies;
    } catch (error) {
  ...
  // Tìm kiếm phim theo thể loại (optimized version)
  getMoviesByCategoryNew: async (category, options = {}) => {
    try {
      ...
      console.log(`Searching movies by category "${category}" - using optimized search`);
      
      // Thử sử dụng API trực tiếp trước
      try {
        const searchParams = {
          page,
          limit,
        ...
        console.log('Trying direct API search with params:', searchParams);
        const data = await movieApi.getNewMovies(page);
        
        if (data.items && Array.isArray(data.items)) {
          // Lọc client-side trong trang hiện tại
          ...
          console.log(`Found ${filteredMovies.length} movies in current page for category "${category}"`);
          
          // Nếu không đủ kết quả, tìm thêm trong cache
          if (filteredMovies.length < limit) {
            console.log('Not enough results, searching in cached movies...');
            const allMovies = await movieApi.getAllMovies();
            const allFiltered = allMovies.filter(movie => {
              const movieCategories = movie.category || [];
              const categoryNames = movieCategories.map(cat => cat.name || cat).join(' ').toLowerCase();
              return categoryNames.includes(category.toLowerCase());
            });
            ...
            const startIndex = (page - 1) * limit;
            const endIndex = startIndex + limit;
            const paginatedMovies = allFiltered.slice(startIndex, endIndex);
        ...
        console.warn('Direct API search failed, falling back to cached search:', apiError);
      }
