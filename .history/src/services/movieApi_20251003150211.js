...
export const movieApi = {
  ...
  // Tìm kiếm phim theo thể loại (full database search với progress)
  getMoviesByCategoryNew: async (category, options = {}) => {
    try {
      ...
      console.log(`Searching movies by category "${category}" - full database search`);
      
      // Sử dụng getAllMovies với progress callback
      const allMovies = await movieApi.getAllMovies(onProgress);
      console.log(`Searching in ${allMovies.length} movies for category "${category}"`);
      ...
      console.log(`Found ${filteredMovies.length} movies for category "${category}"`);
