...
export const movieApi = {
  ...
  getMoviesByCategoryNew: async (category, options = {}) => {
    try {
      ...
      // Bước 1: Lấy 500 phim đầu tiên ngay lập tức
      console.log('Step 1: Getting first 500 movies instantly...');
      const quickMovies = await movieApi.getAllMovies();
      const quickFiltered = quickMovies.filter(movie => {
        const movieCategories = movie.category || [];
        const categoryNames = movieCategories.map(cat => cat.name || cat).join(' ').toLowerCase();
        return categoryNames.includes(category.toLowerCase());
      });
      
      console.log(`Quick search found ${quickFiltered.length} movies in first batch`);
      
      // Bước 2: Trả về kết quả ngay lập tức (500 phim đầu)
      const instantResults = {
        items: quickFiltered.slice(0, 500), // Chỉ lấy 500 phim đầu
        pagination: {
